...
<head>
    ...
    <title>CDA Rastrillantas - Verificar Email</title>
    ...
    <main class="flex-grow flex flex-col relative overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-md mx-auto px-6 pt-12 pb-8 flex flex-col items-center justify-center min-h-[calc(100vh-128px)]">
            ...
            <div class="w-full mb-8 text-left">
                <h1 class="font-headline text-4xl font-extrabold text-tertiary mb-2 tracking-tight">Verificar Correo</h1>
                <p class="font-body text-on-surface-variant text-sm tracking-wide leading-relaxed">
            ...
            @if (session('status') == 'verification-link-sent')
                <div class="w-full mb-8 p-4 bg-tertiary-fixed rounded-lg border-l-4 border-primary-fixed-dim">
                    <p class="font-body text-sm text-tertiary font-semibold">
            ...
            <div class="w-full space-y-6">
                <form method="POST" action="{{ route('verification.send') }}">
                    ...
                    <button type="submit" class="w-full py-4 bg-primary-fixed-dim hover:opacity-90 transition-all duration-150 transform active:scale-[0.98] rounded-lg shadow-[0_24px_40px_rgba(24,28,30,0.06)] flex items-center justify-center gap-2">
                        <span class="font-label text-sm uppercase tracking-[0.1em] font-bold text-on-primary-fixed">Reenviar Enlace de Verificación</span>
                        <span class="material-symbols-outlined text-on-primary-fixed text-lg">send</span>
                    </button>
                </form>

                <form method="POST" action="{{ route('logout') }}" class="text-center pt-2">
                    @csrf
                    <button type="submit" class="font-label text-xs uppercase tracking-wider text-secondary font-semibold hover:text-error transition-colors duration-300">
                        Cerrar Sesión
                    </button>
                </form>
            </div>

...
<head>
    ...
    <title>CDA Rastrillantas - Confirmar Contraseña</title>
    ...
    <main class="flex-grow flex flex-col relative overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-md mx-auto px-6 pt-12 pb-8 flex flex-col items-center justify-center min-h-[calc(100vh-128px)]">
            ...
            <div class="w-full mb-12 text-left">
                <h1 class="font-headline text-4xl font-extrabold text-tertiary mb-2 tracking-tight">Confirmar</h1>
                <p class="font-body text-on-surface-variant text-sm tracking-wide">Esta es un área segura de la aplicación. Por favor, confirma tu contraseña antes de continuar.</p>
            </div>

            <!-- Form Container -->
            <form method="POST" action="{{ route('password.confirm') }}" class="w-full space-y-8">
                @csrf
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full py-4 bg-primary-fixed-dim hover:opacity-90 transition-all duration-150 transform active:scale-[0.98] rounded-lg shadow-[0_24px_40px_rgba(24,28,30,0.06)] flex items-center justify-center gap-2">
                        <span class="font-label text-sm uppercase tracking-[0.1em] font-bold text-on-primary-fixed">Confirmar Contraseña</span>
                        <span class="material-symbols-outlined text-on-primary-fixed text-lg">check_circle</span>
                    </button>
                </div>
            </form>
        </div>

...
@php
    ...
    $usuariosData = $usuarios->map(function($u) {
        return [
            ...
            'username' => $u->username,
            'email' => $u->email,
            'ndocper' => $u->persona->ndocper ?? $u->empresa->nonitem ?? 0,
            'tdocper' => $u->persona->tdocper ?? 10, // 10 es NIT según otros seeders
            'telper' => $u->persona->telper ?? $u->empresa->telem ?? '',
            'actper' => $u->persona->actper ?? 1,
            'nompef' => $u->persona->perfil->nompef ?? $u->empresa->perfil->nompef ?? 'Sin Rol',
            'idpef' => $u->persona->idpef ?? $u->empresa->idpef ?? '',
            ...
            <form :action="'{{ url($mupBase . '/entidades/mup/usuarios') }}/' + selectedUser?.id" method="POST" class="p-8 overflow-y-auto flex-1 custom-scrollbar">
                ...
                <div class="mt-10 flex gap-4">
                    ...
                    <button type="submit" class="flex-1 bg-amber-500 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-amber-500/20 transition-all hover:scale-[1.02]"
                        :disabled="password.length > 0 && (!passwordsMatch || !isPasswordValid)"
                        :class="(password.length > 0 && (!passwordsMatch || !isPasswordValid)) ? 'opacity-50 cursor-not-allowed' : ''">
                        Guardar Cambios
                    </button>
                </div>
            </form>
